 ...
 *
 *  Modified by D.Brundu to perform the de-convolution
 */

#ifndef DECONVOLUTIONFUNCTOR_H_
                ...
                >
{
	...
	typedef DeConvolutionFunctor< Functor, Kernel, detail::BackendPolicy<BACKEND>,
        	 detail::FFTPolicy< typename std::common_type<
        	 	 typename Functor::return_type,
        	 	 typename Kernel::return_type>::type, FFT>,
        	 ArgType>  this_type;
...
typedef typename detail::convolution::_traits<hydra_thrust::tuple<Functor, Kernel>, ArgType>::return_type return_t;
	...
	DeConvolutionFunctor() = delete;

	DeConvolutionFunctor( Functor const& functor, Kernel const& kernel,
	...
	{
		//std::cout << ">>DeConvolutionFunctor()"<<std::endl;
	...
	__hydra_host__ __hydra_device__
	DeConvolutionFunctor( this_type const& other):
	super_type(other),
	...
	virtual void Update() override
	{
		...
		hydra::deconvolute(fft_system_type(), fft_type(),
...
void Dispose(){
	...
	virtual ~DeConvolutionFunctor()=default;
...
inline typename std::enable_if< std::is_floating_point<T>::value, DeConvolutionFunctor<Functor, Kernel,
                 detail::BackendPolicy<BACKEND>, detail::FFTPolicy<T, FFT>, ArgType>>::type
make_deconvolution( detail::BackendPolicy<BACKEND> const&, detail::FFTPolicy<T, FFT> const&, Functor const& functor, Kernel const& kernel,
		T kmin, T kmax, unsigned nsamples=1024,	bool interpolate=true, bool power_up=true)
{
	return DeConvolutionFunctor<Functor, Kernel,
