...
struct DivideFFT
{
	...
	operator()(hydra::tuple<complex_type, complex_type> const& points){
		...
		return hydra::get<1>(points)/hydra::get<0>(points);
	}
};
...
,void>::type
deconvolute(detail::BackendPolicy<BACKEND> policy, detail::FFTPolicy<T, FFTBackend> fft_policy,
	...
	// sample kernel
	auto kernel_sampler = hydra::detail::convolution::FunctorSampler<Kernel>(kernel, nsamples, min, delta);
	...
	hydra_thrust::transform( policy, ffts.begin(),  ffts.end(),
			complex_buffer.first, detail::convolution::DivideFFT<T>());
