...
decW = tk.Tk();
tfb_ui.apply_font_scaling(decW)
...
def selectoutput():
    ...
    if not filechosen1:               # dialog cancelled
        return
    #filelabel = Label(root, text=filechosen,font = ("Arial",))
    #directory = os.path.split(filechosen)[0] + '/' + os.path.split(filechosen)[1]
    text1 = Text(decW, state='disabled',font = ("Arial",10), width=50, height=1)
...
def writeCFG( ):
    ...
    text_file.write('token2    =' + '"' + token_entry2.get() + '";\n')
    ...
    subprocess.run(["./deconvolution_tbb"], cwd="../build")
...
def run_results ():
    ...
    if (varlt.get()==0):
        data1 = pd.read_csv(os.path.split(filechosen.name)[0] + '/' + os.path.split(filechosen.name)[1],sep=r'\s+',header=None)
        data1 = pd.DataFrame(data1)
        data2 = pd.read_csv(os.path.split(filechosen1.name)[0] + '/' + os.path.split(filechosen1.name)[1],sep=r'\s+',header=None)
        data2 = pd.DataFrame(data2)

    if (varlt.get()==1):
        data1 = pd.read_csv(os.path.split(filechosen.name)[0] + '/' + os.path.split(filechosen.name)[1],sep=r'\s+',header=None, skiprows=1)
        data1 = pd.DataFrame(data1)
        data2 = pd.read_csv(os.path.split(filechosen1.name)[0] + '/' + os.path.split(filechosen1.name)[1],sep=r'\s+',header=None, skiprows=1)
        data2 = pd.DataFrame(data2)
    ...
    data3 = pd.read_csv(os.path.split(folder_selected3)[0] + '/' + os.path.split(folder_selected3)[1] +'/'+ str(nameentry.get()),sep=r'\s+',header=None)
...
my_label7 = Label(decW, text='TFBOOST DECONVOLUTION GUI',font = ("Arial",9))
my_label7.place(x=20,y=85)

my_canvas0 = Canvas(decW,width=750,height=1,bg='black')
my_canvas0.place(x=20,y=100)

lab2 = Label(decW, text='Perform deconvolution with TFBoost using a input signal and the corresponding output signal:',font = ("Arial",9))
lab2.place(x=30,y=120)
lab2b = Label(decW, text='(the time step is set to 1ps by default)',font = ("Arial",9))
...
btnRun = Button(decW, text="Make \n deconvolution",command=writeCFG,font = ("Arial",12))
